...
struct CleanupRule {
    std::string   pattern;
    CleanupAction action    = CleanupAction::Delete;
...
public:
    ...
    void AddRule(const CleanupRule& rule);
    CleanupReport RunCleanup(const std::string& rootDir = ".");
    CleanupReport GetReport() const;
    const std::vector<CleanupRule>& GetRules() const;
...
private:
    ...
    std::vector<CleanupRule>               m_rules;
    CleanupReport                          m_lastReport;
};
