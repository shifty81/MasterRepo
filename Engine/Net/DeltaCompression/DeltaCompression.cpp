#include "Engine/Net/DeltaCompression/DeltaCompression.h"
#include <cmath>
...
int32_t DeltaCompression::QuantizePosition(float v)  { return static_cast<int32_t>(std::round(v * kPositionScale)); }
float   DeltaCompression::DequantizePosition(int32_t v) { return static_cast<float>(v) / kPositionScale; }
int32_t DeltaCompression::QuantizeRotation(float deg)   { return static_cast<int32_t>(std::round(deg * kRotationScale)); }
float   DeltaCompression::DequantizeRotation(int32_t v) { return static_cast<float>(v) / kRotationScale; }

CompressedFrame DeltaCompression::Encode(const std::vector<EntitySnapshot>& snapshots) {
    CompressedFrame frame;
    if (!snapshots.empty()) frame.tick = snapshots.front().tick;
    ...
    for (const auto& snap : snapshots) {
        ...
        if (needsKeyframe(snap.entityId, snap.tick)) {
            ...
            bl.snapshot = snap; bl.lastKeyframeTick = snap.tick; bl.forceKeyframe = false;
        } else {
...
std::vector<EntitySnapshot> DeltaCompression::Decode(const CompressedFrame& frame) {
    ...
    for (const auto& cs : frame.entries) {
        ...
        if (cs.frameType == FrameType::Keyframe) {
            ...
            bl.snapshot = snap; bl.lastKeyframeTick = cs.tick;
        } else {
...
void DeltaCompression::ForceKeyframe(uint32_t entityId) {
    ...
    if (it != m_baselines.end()) it->second.forceKeyframe = true;
}

void DeltaCompression::ForceAllKeyframes() {
    m_forceAll = true;
    for (auto& [id, bl] : m_baselines) bl.forceKeyframe = true;
}

void DeltaCompression::RemoveEntity(uint32_t entityId) { m_baselines.erase(entityId); }
void DeltaCompression::Clear()                         { m_baselines.clear(); m_forceAll = false; }

bool DeltaCompression::needsKeyframe(uint32_t entityId, uint32_t currentTick) const {
    if (m_forceAll) return true;
    auto it = m_baselines.find(entityId);
    if (it == m_baselines.end()) return true;
    if (it->second.forceKeyframe) return true;
    return currentTick >= it->second.lastKeyframeTick + m_keyframeInterval;
}
