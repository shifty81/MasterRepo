#include "Builder/Core/BuildAuditLog.h"
#include <fstream>
#include <sstream>
...
void BuildAuditLog::Log(AuditSeverity severity, AuditCategory category,
                        const std::string& actor, const std::string& message,
                        const std::string& detail) {
    if (!m_open) return;
    AuditEntry e;
    e.id          = m_nextId++;
    e.timestampMs = static_cast<uint64_t>(m_entries.size());
    e.severity    = severity;
    e.category    = category;
    e.actor       = actor;
    e.message     = message;
...
void BuildAuditLog::Info   (const std::string& a, const std::string& m)                        { Log(AuditSeverity::Info,    AuditCategory::Build, a, m); }
void BuildAuditLog::Warning(const std::string& a, const std::string& m)                        { Log(AuditSeverity::Warning, AuditCategory::Build, a, m); }
void BuildAuditLog::Error  (const std::string& a, const std::string& m, const std::string& d)  { Log(AuditSeverity::Error,   AuditCategory::Build, a, m, d); }
void BuildAuditLog::Fatal  (const std::string& a, const std::string& m, const std::string& d)  { Log(AuditSeverity::Fatal,   AuditCategory::Build, a, m, d); }
...
std::vector<AuditEntry> BuildAuditLog::FilterBySeverity(AuditSeverity sev) const {
    ...
    for (const auto& e : m_entries) if (e.severity == sev) out.push_back(e);
    return out;
}
std::vector<AuditEntry> BuildAuditLog::FilterByCategory(AuditCategory cat) const {
    std::vector<AuditEntry> out;
    for (const auto& e : m_entries) if (e.category == cat) out.push_back(e);
    return out;
}
std::vector<AuditEntry> BuildAuditLog::FilterByActor(const std::string& actor) const {
    std::vector<AuditEntry> out;
    for (const auto& e : m_entries) if (e.actor == actor) out.push_back(e);
    return out;
}
...
std::string BuildAuditLog::ExportText() const {
    ...
    for (const auto& e : m_entries) {
        oss << "[" << SeverityLabel(e.severity) << "][" << CategoryLabel(e.category)
            << "][" << e.actor << "] " << e.message;
...
std::string BuildAuditLog::ExportJSON() const {
    std::ostringstream oss;
    oss << "{\"build_id\":\"" << m_buildId << "\",\"entries\":[\n";
...
void BuildAuditLog::SetEntryCallback(EntryCallback cb) { m_callback = std::move(cb); }
void BuildAuditLog::Close(uint64_t durationMs)         { m_durationMs = durationMs; m_open = false; }
void BuildAuditLog::Reset() {
    m_open = false; m_buildId.clear(); m_entries.clear(); m_nextId = 1; m_durationMs = 0; m_callback = nullptr;
}
